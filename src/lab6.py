    ...
    def word(self):
        ...
        while self.i < len(self.s):
            if self.s[self.i] == "(":
                ...
                paren_depth -= 1
                self.i += 1
            elif self.s[self.i].isalnum() or self.s[self.i] in "#-.%*/_":
                self.i += 1
            elif paren_depth > 0 and self.s[self.i] in ", ":
                self.i += 1
            else:
                break
        if not (self.i > start):
...
_CSS_PATH = os.path.join(os.path.dirname(__file__), "browser6.css")
DEFAULT_STYLE_SHEET = CSSParser(open(_CSS_PATH).read()).parse()
